...
import { z } from "zod";
import { Lock, Mail, Phone, ShieldCheck, ArrowLeft } from "lucide-react";
    ...
    try {
      const { error } = await supabase.auth.signInWithOtp({
        ...
        options: {
          ...
          data: isSignup ? { display_name: displayName.trim() || undefined } : undefined,
        },
      });
      if (error) throw error;
      setOtpSent(true);
    ...
    <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-gradient-to-b from-background to-secondary/60 px-6 py-12">
      ...
      <div className="relative w-full max-w-sm space-y-6">
        ...
        <div className="rounded-3xl bg-card p-5 shadow-card animate-slide-up">
          <Tabs defaultValue="email" className="w-full" onValueChange={() => { setOtpSent(false); setOtp(""); }}>
            <TabsList className="grid w-full grid-cols-2 rounded-xl">
              <TabsTrigger value="email" className="rounded-lg"><Mail className="mr-1.5 h-3.5 w-3.5" />Email</TabsTrigger>
              <TabsTrigger value="phone" className="rounded-lg"><Phone className="mr-1.5 h-3.5 w-3.5" />Phone</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="phone" className="mt-4">
              {!otpSent ? (
                <form onSubmit={handleSendOtp} className="space-y-4">
                  {isSignup && (
                    <div className="space-y-1.5">
                      ...
                      <Input id="name-p" value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="Alex Carter" />
                    </div>
                  )}
                  <div className="space-y-1.5">
                  ...
                  <div className="flex items-center justify-center gap-1.5 rounded-xl bg-secondary p-2.5 text-[11px] text-muted-foreground">
                    <ShieldCheck className="h-3.5 w-3.5 text-accent" />
                    Secured by one-time code · Never shared
                  </div>
                </form>
              ) : (
                <form onSubmit={handleVerifyOtp} className="space-y-5 animate-slide-up">
                  ...
                  <Button type="submit" className="w-full h-12 rounded-xl text-base font-semibold" disabled={loading || otp.length !== 6}>
